...
SS_NAME='03_NFIP_2018_RMSv17_StormSurge_ExposureReport.xlsx'
TABLE_NAME='rmsv17_stormsurge_exposure'

# This sheet really contains a bunch of sub tables.
# We output each of them to a separate CSV.
in2csv --sheet 'RMSv17 Exposure Summary' "${IN_DIR}/${SS_NAME}" 2>/dev/null |
  tail -n+11 |
  sed '
...
in2csv --sheet 'RMSv17 Occupancy by State' "${IN_DIR}/${SS_NAME}" 2>/dev/null |
  tail -n+9 |
  sed "$SED_CLEAN_TBL_SCRIPT" |
  gzip -9 \
> "${OUT_DIR}/${TABLE_NAME}_by_occupancy_code.csv.gz"

in2csv --sheet 'RMSv17 Deductible Profiles' "${IN_DIR}/${SS_NAME}" 2>/dev/null |
  tail -n+9 |
  sed "$SED_CLEAN_TBL_SCRIPT" |
  gzip -9 \
...
in2csv --sheet 'RMSv17 Deductible Profiles' "${IN_DIR}/${SS_NAME}" 2>/dev/null |
  ...
  sed "$SED_CLEAN_TBL_SCRIPT" |
  gzip -9 \
> "${OUT_DIR}/${TABLE_NAME}_content_deductibles.csv.gz"
