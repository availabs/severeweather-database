...
DROP TABLE IF EXISTS hazard_loans.__TABLE_NAME__ CASCADE;
...
CSV_PATH="$(readlink -e ../../../data/hazard_loans/__TABLE_NAME__.csv.gz)"
...
gunzip -c "$CSV_PATH" | psql -c "
  COPY hazard_loans.__TABLE_NAME__ (${header})
  ...
  CREATE_TABLE_DDL="$(
    ...
    csvsql -i postgresql --tables "${TABLE_NAME}" --no-constraints 2>/dev/null |
        sed "
          ...
          s/DATE/VARCHAR/g;            # Change DATEs datatype to VARCHARs because dates are borked
          s/${TABLE_NAME}/hazard_loans.&/g; # Add the schema to the column name
        "
  )"
