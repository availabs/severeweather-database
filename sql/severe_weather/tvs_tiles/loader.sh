...
DATADIR="$(readlink -e ../../../data/severe_weather/tvs-tiles)"

psql -f ./create_tvs_tiles.sql
...
do
  ...
  TABLE="\"severe_weather_data\".tvs_tiles_${YEARMO}"
  echo "$TABLE"

  if ! psql -c "\d ${TABLE}" > /dev/null 2>&1 || "$CLOBBER"
  then
    psql -v YEAR="$YEARMO" -f ./load_tvs_tiles.sql
