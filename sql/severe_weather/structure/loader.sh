...
DATADIR="$(readlink -e ../../../data/severe_weather/structure)"

psql -f ./create_structure.sql
...
do
  ...
  TABLE="\"severe_weather_data\".structure_${YEARMO}"
  echo "$TABLE"

  if ! psql -c "\d ${TABLE}" > /dev/null 2>&1 || "$CLOBBER"
  then
    psql -v YEAR="$YEARMO" -f ./load_structure.sql
